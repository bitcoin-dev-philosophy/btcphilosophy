...
as multiple entities. But as the quote also mentions, the Sybil attack
is not effective on the Bitcoin network, because you're not in any way
voting with nodes or entities, but <<one-cpu-one-vote,with computing
power>>, but it can help carrying out other attacks.
