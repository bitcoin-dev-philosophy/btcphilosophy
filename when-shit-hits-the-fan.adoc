...
Apparently, the two int64 (not uint64, as Garzik supposed) outputs' sum
...
There was a big caveat with this: all the data users would download from
...
the previous OP_RETURN case: this issue was exploited [comm: do you mean "exposed"? If someone had the chance to exploit the bug, it isn't clear from the text] in the wild, and
thus a fix had to be made more quickly. In the case of
...
_atomically_, meaning that they should be complete enough to provide something useful or solve a
