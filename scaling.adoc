...
Scaling has been a very hot topic since the early days of Bitcoin, but
...
building protocols on top of Bitcoin in layers.
...
* Delay validation of an incoming block, also known as _validationless
  mining_. Just check the block header's proof-of-work and mine on top
  of it while you download the full block and validate it.
...
image::Bitcoin-Core-Sync-Performance-1.png[{half-width}]
...
payment. This can save you a lot of fees, while reducing the block
space load.

.Transaction batching combines multiple payments into a single transaction to save on fees.
image::tx-batching.png[{half-width}]
...
saves on fees later when you want to make a payment during high
block space demand.
...
track of who owns which coins, decreases because you spend more coins
