...
We can thus be sure that the BTC supply is at most
...
The finite supply will shrink which, at least in theory, should
cause price deflation.

More important than the exact number of coins in circulation is
how the supply limit is enforced without any central
...
<<combined-output-overflow,When shit hits the fan>> chapter), run evil
softwares, yet the collective will robustly secure the blockchain.
Thus, you can choose to trust the system without having to
trust anyone.
...
say for sure that with today's conditions with regard to block subsidy, transaction fees,
...
countermeasures that have been taken, such as relying on transaction
...
which fixed a very-long-term inflation bug - be zero from around year
...
discussion in 10-50 years, when we'll know more. If it will still be relevant.
