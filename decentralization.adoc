...
If they succeed, the same issues will most likely resurface at some point in the future because the system
remained as censorable as it was before.
...
miners' hash power to avoid any 51% attacks:
...
In the paragrahs above, we've mostly talked about miner decentralization and how
...
someone else will. Now suppose that most users are delegating consensus enforcement to a
...
likely lead to miners having to sign legal contracts in which they agree
