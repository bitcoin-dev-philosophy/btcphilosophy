...
throughout the years, up until 2017. Since then, only one other major upgrade, which we analyze in <<taproot-deployment>>, has been deployed. The long and somewhat chaotic process that led to its activation has helped us gaining further insights on upgrading mechanisms in Bitcoin. 
